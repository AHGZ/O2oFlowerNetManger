...
/**
 * 名称：AppPartnerAlertDialog.java
 * 描述：自定义弹框
 * 创建人：zhangpeisen
 * 创建时间：2016/12/16 14:55
 */
public class AppPartnerAlertDialog {
    ...
    public AppPartnerAlertDialog(Activity activity) {
    ...
    public AppPartnerAlertDialog builder() {
        // 获取Dialog布局
        View view = LayoutInflater.from(context).inflate(R.layout.app_partner_dialog, null);
    ...
    public AppPartnerAlertDialog setTitle(String title) {
        showTitle = true;
        if ("".equals(title)) {
    ...
    public AppPartnerAlertDialog setMsg(String msg) {
        showMsg = true;
        if ("".equals(msg)) {
            txt_msg.setText("内容");
        } else {
    ...
    public AppPartnerAlertDialog setCancelable(boolean cancel) {
        dialog.setCancelable(cancel);
        return this;
    }

    public AppPartnerAlertDialog setPositiveButton(String text, final OnClickListener listener) {
        showPosBtn = true;
        if ("".equals(text)) {
            btn_pos.setText("确定");
        } else {
    ...
    public AppPartnerAlertDialog setNegativeButton(String text, final OnClickListener listener) {
        showNegBtn = true;
        if ("".equals(text)) {
            btn_neg.setText("取消");
        } else {
