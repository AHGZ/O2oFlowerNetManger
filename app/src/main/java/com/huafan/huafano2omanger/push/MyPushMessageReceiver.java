...
import com.huafan.huafano2omanger.utils.PushUtils;
...
public class MyPushMessageReceiver extends PushMessageReceiver {
    ...
    public void onBind(Context context, int errorCode, String appid,
        ...
        SPUtils.put(context, "devicetoken", channelId);
    ...
    private void updateView(String type) {
        if (!TextUtils.isEmpty(type)) {
            //您有新的花返退款申请,待处理
            if ("20".equals(type)) {
                mediaPlayer = MediaPlayer.create(mContext, R.raw.refund_money);
                playVoice(mContext);
                //您有新的花返外卖订单,请及时处理
                ...
                mediaPlayer = MediaPlayer.create(mContext, R.raw.automic_order);
                playVoice(mContext);
                //您有新的花返推送消息,请查收
            } else if ("22".equals(type)) {
                mediaPlayer = MediaPlayer.create(mContext, R.raw.new_push);
                playVoice(mContext);
            }
        }
    }
    ...
    public void onNotificationArrived(Context context, String title,
        ...
        Log.e("TAG", "notifyString==" + notifyString);

        // 自定义内容获取方式，mykey和myvalue对应通知推送时自定义内容中设置的键和值
        EventBus.getDefault().post(new PushMsgEvent(title, customContentString,description));
//        String myvalue = null;
//
...
//                if (!customJson.isNull("type")) {
//
//                    Log.e("TAG", "customJson==" + customJson);
//                    myvalue = customJson.getString("type");
...
//        updateView(myvalue);
    ...
    public void onNotificationClicked(Context context, String title,
        ...
        Log.e("TAG", "notifyString11==" + notifyString);
        EventBus.getDefault().post(new PushMsgEvent(title, customContentString, description));
        // 自定义内容获取方式，mykey和myvalue对应通知推送时自定义内容中设置的键和值
        if (!TextUtils.isEmpty(customContentString)) {
            JSONObject customJson = null;
    ...
    private void updateContent(Context context, String content) {
        Log.e("TAG", "updateContent==" + content);
