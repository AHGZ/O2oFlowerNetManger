...
public class Utils {
    ...
    /**
     * useThemestatusBarColor : 0代表渐变色 ，1代表白色,2代表深绿色
     *
     * @param activity
     * @param useThemestatusBarColor   是否要状态栏的颜色，不设置则为透明色
     * @param withoutUseStatusBarColor 是否不需要使用状态栏为暗色调
     */
    public static void setStatusBar(final Activity activity, int useThemestatusBarColor, boolean withoutUseStatusBarColor) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0及以上
            ...
            if (useThemestatusBarColor == 0) {
                ...
                Looper.myQueue().addIdleHandler(new MessageQueue.IdleHandler() {
                    ...
                    public boolean queueIdle() {
                        if (isStatusBar()) {
                            new StatusUtil(activity).initStatusBar(0);
            ...
            } else if (useThemestatusBarColor == 1) {
                //1代表白色
                //延时加载数据.
                Looper.myQueue().addIdleHandler(new MessageQueue.IdleHandler() {
                    @Override
                    public boolean queueIdle() {
                        if (isStatusBar()) {
                            new StatusUtil(activity).initStatusBar(1);
                            activity.getWindow().getDecorView().addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
                                @Override
                                public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft,
                                                           int oldTop, int oldRight, int oldBottom) {
                ...
                Looper.myQueue().addIdleHandler(new MessageQueue.IdleHandler() {
                    ...
                    public boolean queueIdle() {
                        if (isStatusBar()) {
                            ...
                            new StatusUtil(activity).initStatusBar(2);
                            activity.getWindow().getDecorView().addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
                                @Override
                                public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft,
                                                           int oldTop, int oldRight, int oldBottom) {
